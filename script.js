(function () {
  ...
  document.getElementById("deck").addEventListener("click", (e) => {
    if (e.target.closest("a, button, .plant-card, .fauna-card, .flip-card, .toggle-btn, .photo-trigger")) return;
    const x = e.clientX;
  ...
  // parallax/tilt do relevo (slide "relevo e solo") — a foto real reage ao mouse
  const terrainVisual = document.getElementById("terrainVisual");
  const terrainPhoto = document.getElementById("terrainPhoto");
  if (terrainPhoto && window.PHOTOS && PHOTOS.relevo) {
    terrainPhoto.src = PHOTOS.relevo;
  }
  if (terrainVisual && terrainPhoto) {
    terrainVisual.addEventListener("mousemove", (e) => {
      const rect = terrainVisual.getBoundingClientRect();
      const relX = (e.clientX - rect.left) / rect.width - 0.5;
      const relY = (e.clientY - rect.top) / rect.height - 0.5;
      terrainPhoto.style.transform =
        "rotateY(" + (relX * 10) + "deg) rotateX(" + (relY * -10) + "deg) scale(1.04)";
    });
    terrainVisual.addEventListener("mouseleave", () => {
      terrainPhoto.style.transform = "rotateY(0) rotateX(0) scale(1)";
    });
  }
  ...
  function openLightbox(photoKeys, caption, desc) {
    const keys = String(photoKeys || "").split(",").map((k) => k.trim()).filter(Boolean);
    lightboxImages.innerHTML = "";
    keys.forEach((key) => {
      const src = (window.PHOTOS || {})[key];
      if (!src) return;
  ...
  document.querySelectorAll(".photo-trigger").forEach((el) => {
    el.addEventListener("click", (e) => {
      e.stopPropagation();
      openLightbox(el.dataset.photo, el.dataset.caption, el.dataset.desc);
    });
